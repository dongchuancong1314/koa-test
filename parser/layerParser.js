...
const handleBorderRadius = function (points) {
  return points.map((point) => point.cornerRadius).join(" ");
};
...
const handleItem = function (item) {
  ...
  if (item.points) {
    result.style.borderRadius = handleBorderRadius(item.points);
  }
